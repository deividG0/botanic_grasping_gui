...
class JointStatePublisher(Node):
    def __init__(self):
        ...
        self.joint_limits = {'joint1': (-np.pi/2, np.pi/2),
                             'joint2': (-0.25, 0.35),
                             'joint3': (0.25, 1.05),
    ...
    def get_random_point(self):
        ...
        operating_space_min = [0.35, -0.25, 0.2]
        # Replace with your max bound coordinates
        operating_space_max = [0.45, 0.25, 0.5]
        ...
        marker.scale.x = operating_space_max[0] - operating_space_min[0]
        marker.scale.y = operating_space_max[1] - operating_space_min[1]
        marker.scale.z = operating_space_max[2] - operating_space_min[2]
        marker.color.a = 0.6  # Alpha value (opacity)
        marker.color.r = 0.0
        marker.color.g = 0.0
        marker.color.b = 1.0
        ...
        marker.pose.orientation.w = self.target_orientation[0]
        marker.pose.orientation.x = self.target_orientation[1]
        marker.pose.orientation.y = self.target_orientation[2]
        marker.pose.orientation.z = self.target_orientation[3]
    ...
    def execute_trajectory(self):
        ...
        goal.trajectory.points.append(
            JointTrajectoryPoint(positions=list(self.joint_angles) + [0.2],
                                 velocities=[0.0]*7,
                                 accelerations=[0.0]*7,
    ...
    def is_ik_acceptable(self, c, joint_angles,
                         required_position, required_orientation):
        """
        ...
        distance = \
            math.sqrt((fk_position[0] - required_position[0])**2
                      + (fk_position[1] - required_position[1])**2
                      + (fk_position[2] - required_position[2])**2)
        if distance < 0.001 and \
            required_orientation[0]-offset_orientation < fk_rot[0] \
            < required_orientation[0]+offset_orientation and \
                required_orientation[1]-offset_orientation < fk_rot[1] \
            < required_orientation[1]+offset_orientation and \
                required_orientation[2]-offset_orientation < fk_rot[2] \
            < required_orientation[2]+offset_orientation and \
                required_orientation[3]-offset_orientation < fk_rot[3] \
    ...
    def is_joint_valid(self, target):
        if self.joint_limits['joint1'][0] < target[0] \
            < self.joint_limits['joint1'][1] and \
            self.joint_limits['joint2'][0] < target[1] \
            < self.joint_limits['joint2'][1] and \
                self.joint_limits['joint3'][0] < target[2] \
            < self.joint_limits['joint3'][1] and \
                self.joint_limits['joint4'][0] < target[3] \
            < self.joint_limits['joint4'][1] and \
                self.joint_limits['joint5'][0] < target[4] \
            < self.joint_limits['joint5'][1] and \
                self.joint_limits['joint6'][0] < target[5] \
